...
class CrossDomainClassSpecificFrequencyMixStyle(nn.Module):
    def __init__(self, f_dim, num_domains, num_classes, p=0.5, alpha=0.5, eps=1e-6, mix='crossdomain', class_mix=True):
        super().__init__()
        self.f_dim = f_dim
        self.p = p
        self.alpha = alpha
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        # print("PatchEmbed input:", x.min().item(), x.max().item(), x.mean().item())
        x = self.proj(x).flatten(2).transpose(1, 2)
        # print("PatchEmbed output:", x.min().item(), x.max().item(), x.mean().item())
        return x
        ...
        if training:
            if args.adversarial_ft:
                if args.device_mode in ['mixed']:    
                    device_x = ReverseLayerF.apply(x, alpha)
                    x = (x, device_x)
                else:
                    device_x = ReverseLayerF.apply(x, alpha)
                    x = (x, device_x)
