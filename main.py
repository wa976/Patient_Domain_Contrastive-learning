...
from util.debuging import NaNDebugger, check_nan, print_stats
from models import get_backbone_class, Projector
from method import PatchMixLoss, PatchMixConLoss,PDCLoss,DeepCORALLoss
...
def set_loader(args):
    ...
    if args.balance_sampler:
        ...
        domain_ratios = {0: 1, 1: 2}  # 0은 A 도메인, 1은 B 도메인을 나타낸다고 가정
...
def set_model(args):
        ...
        if ckpt.get('classifier', None) is not None:
            ...
            if args.adversarial_ft:
                if args.device_mode in ['mixed']:
                    ...
                    device_discriminator.load_state_dict(ckpt['classifier'], strict=True)
                else:
                    class_classifier.load_state_dict(ckpt['classifier'], strict=True)
                    data_discriminator.load_state_dict(ckpt['classifier'], strict=True)
                    device_discriminator.load_state_dict(ckpt['classifier'], strict=True)
            else:    
    ...
    elif args.method == 'colar':
        criterion = [criterion.cuda(), DeepCORALLoss().cuda()]
    elif args.method == 'pdc':
        criterion = [criterion.cuda(), PDCLoss(temperature=args.temperature).cuda()]
            ...
            if args.method == 'ce':
                ...
                if args.nospec:
                    ...
                    if args.frequency_stylemix:
                        features = model(args.transforms(images), args=args, alpha=lamb, training=True,frequency_stylemix=True,domain_labels=device_labels,class_labels=class_labels)
                    else:
                        features = model(args.transforms(images), args=args, alpha=lamb, training=True,frequency_stylemix=False,domain_labels=device_labels,class_labels=class_labels)
                    # print("input shape : ", images.shape)
                    # print("target shape : ", class_labels.shape)
                    # print("outputs shape : ", features.shape)
                    
                    # 디버깅: features 체크
                    if check_nan(features, "features"):
                        raise RuntimeError("NaN detected in features")
                    print_stats(features, "features")
                ...
                if args.adversarial_ft:
                    if args.device_mode in ['mixed']:
                        ...
                        loss = class_loss + args.alpha * device_loss
                else:
                ...
                if args.nospec:
                    ...
                    for i in range(len(device_labels)):
                        if device_labels[i].item() == 0:
                            features = model(args.transforms(images), args=args, alpha=lamb, training=True)
                            source_feature.append(features)
                        else:
                            features = model(args.transforms(images), args=args, alpha=lamb, training=True)
                            target_feature.append(features)
                    
                output = classifier(features)
                class_loss = criterion[0](output, class_labels)
                
                # Initialize source_features and target_features as empty tensors
                source_features = torch.tensor([])
                target_features = torch.tensor([])
                
                
                # Check if source_features and target_features are non-empty lists of tensors
                if isinstance(source_feature, list) and len(source_feature) > 0:
                    source_features = torch.stack(source_feature)
                if isinstance(target_feature, list) and len(target_feature) > 0:
                    target_features = torch.stack(target_feature)

                # Now calculate the CORAL loss, making sure that both source_features and target_features are not empty
                if source_features.size(0) > 0 and target_features.size(0) > 0:
                    colar_loss = criterion[1](source_features, target_features)
                else:
                    colar_loss = torch.tensor(0.0)
                
                loss = class_loss + args.colar_weight * colar_loss  
                ...
                if args.nospec:
                    ...
                    # Compute PDCLoss
                    pdc_loss = criterion[1](features, class_labels, patient_labels, device_labels)


                    # Combine class loss and PDC loss
                    loss = args.ce_weight * class_loss + args.pdc_weight * pdc_loss
